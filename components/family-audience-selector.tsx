...
import { X, ChevronDown, Mail, ChevronUp } from 'lucide-react'
...
export function FamilyAudienceSelector({
  ...
  // Debounced search — always search regardless of @ in input
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="space-y-2">
      ...
      <div className="relative">
        ...
        {/* Search dropdown — shown while typing 2+ chars */}
        {open && inputValue.trim().length >= 2 && (
          <div className="absolute z-50 mt-1 w-full rounded-lg border border-zinc-600 bg-zinc-800 shadow-2xl overflow-hidden ring-1 ring-black/40">
            ...
            {searching ? (
              ...
              // No results — if it's a valid email offer to add directly
              emailIsValid ? (
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="text-zinc-300">Add <span className="text-blue-300 font-medium">{inputValue.trim().toLowerCase()}</span> directly</span>
                </button>
              ) : (
                <div className="px-3 py-2.5 text-sm text-zinc-500">No families found matching "{inputValue.trim()}"</div>
              )
            )}

            {/* Always offer "add raw email" at the bottom when input is a valid email and there ARE results */}
            {!searching && searchResults.length > 0 && emailIsValid && (
              <div className="border-t border-zinc-700">
                <button
                  type="button"
                  onMouseDown={(e) => { e.preventDefault(); addEmail(inputValue) }}
                  className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-zinc-700 transition-colors text-zinc-400"
                >
                  <Mail className="h-3.5 w-3.5 text-blue-400 flex-shrink-0" />
                  <span>Add <span className="text-blue-300">{inputValue.trim().toLowerCase()}</span> as direct email</span>
                </button>
              </div>
            )}
          </div>
      ...
      {/* Browse by program — stable section, not inside a floating overlay */}
      <div className="rounded-lg border border-zinc-700 bg-zinc-900">
        <button
          type="button"
          onClick={() => setBrowseOpen(v => !v)}
          className={cn(
            'w-full flex items-center justify-between px-3 py-2 text-sm text-zinc-400 hover:text-zinc-300 hover:bg-zinc-800/60 transition-colors',
            browseOpen ? 'rounded-t-lg' : 'rounded-lg'
          )}
        >
        ...
        {browseOpen && (
          <div className="border-t border-zinc-700 px-3 py-3 space-y-2.5 bg-zinc-800/30 rounded-b-lg">
            {loadingPrograms ? (
            ...
            {browseTarget && (
              loadingBrowse ? (
                ...
                <>
                  ...
                  <ul className="max-h-52 overflow-y-auto rounded-lg border border-zinc-700 divide-y divide-zinc-700/60">
